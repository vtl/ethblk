...
static void ethblk_target_unregister_netdevice(struct net_device *nd)
{
	...
	rcu_read_lock();
	xa_for_each(&ethblk_target_disks, drv_id, d) {
...
// FIXME review and remove		ethblk_target_put_disk(d);
	...
	xa_store(&ethblk_target_disks, d->drv_id, d, GFP_KERNEL);
...
static void ethblk_target_checksum_cmd_free(struct ethblk_target_checksum_cmd *cs_cmd)
{
	dprintk(debug, "cs_cmd %px\n", cs_cmd);
	...
	if (cs_cmd->bio)
		bio_put(cs_cmd->bio);
	if (cs_cmd->data)
		free_pages((unsigned long)cs_cmd->data, cs_cmd->order);
	kfree(cs_cmd);
}

static void ethblk_target_cmd_checksum_complete(struct bio *bio)
{
	char *p;
	int i;
	...
	if (!ethblk_worker_enqueue(workers, &cs_cmd->list)) {
		dprintk_ratelimit(debug, "can't enqueue bio work\n");
		goto error;
	}
	return;
...
static void ethblk_target_checksum_cmd_iter(struct ethblk_target_checksum_cmd *cs_cmd)
{
	...
	bio->bi_iter.bi_sector = cs_cmd->lba;
	bio_set_dev(bio, d->bd);
	bio_set_op_attrs(bio, REQ_OP_READ, 0);
	bio->bi_end_io = ethblk_target_cmd_checksum_complete;
	bio->bi_private = cs_cmd;
	cs_cmd->bio = bio;
...
static void ethblk_target_cmd_checksum(struct ethblk_target_cmd *cmd)
{
	ssize_t rep_skb_alloc_len;
	struct ethblk_hdr *req_hdr;
	struct ethblk_hdr *rep_hdr;
	...
	lba = be64_to_cpu(req_hdr->lba);
	len = SHA_DIGEST_WORDS * 4; /* SHA-1 digest is 160 bits */
	...
	dprintk(debug, "req_skb %px rep_skb %px tag %u disk %s op %s lba %llu num_sectors %d\n",
		req_skb, rep_skb, be32_to_cpu(req_hdr->tag), d->name,
		"CHECKSUM", lba, req_hdr->num_sectors);

	NET_STAT_INC(cmd->ini, cnt.rx_count);

	cs_cmd = kzalloc(sizeof(struct ethblk_target_checksum_cmd), GFP_KERNEL);
	if (!cs_cmd) {
		dprintk(err, "can't alloc checksum cmd\n");
		goto out_err;
	}
	INIT_LIST_HEAD(&cs_cmd->list);
	cs_cmd->cmd = cmd;
	cs_cmd->lba = lba;
	...
	if (!cs_cmd->data) {
		dprintk(err, "can't alloc checksum cmd data\n");
		goto out_err;
	}

	skb_put(rep_skb, len);

	ethblk_target_checksum_cmd_iter(cs_cmd);
	return;
out_err:
	NET_STAT_INC(cmd->ini, cnt.err_count);
	ETHBLK_HDR_SET_STATUS(rep_hdr, 1);
...
void ethblk_target_cmd(struct ethblk_target_cmd *cmd)
{
	...
	case ETHBLK_OP_CHECKSUM:
		ethblk_target_cmd_checksum(cmd);
		break;
	default:
		/* FIXME rx_dropped++ */
		dprintk(err, "skb %px unknown cmd %d\n", cmd->req_skb,
			cmd->req_hdr->op);
		ethblk_target_cmd_free(cmd);
		break;
	}
	...
	for (;;) {
		...
		list_for_each_entry_safe (cmd, n, &queue, list) {
			...
			case ETHBLK_TARGET_CMD_WORK_TYPE_BIO:
				ethblk_target_send_reply(cmd);
				break;
			case ETHBLK_TARGET_CMD_WORK_TYPE_CHECKSUM:
				ethblk_target_checksum_cmd_iter(
					(struct ethblk_target_checksum_cmd *)cmd);
				break;
			default:
...
void ethblk_target_handle_discover(struct sk_buff *skb)
{
	...
	dprintk(info, "checking initiator %s_%pM access to disks\n",
